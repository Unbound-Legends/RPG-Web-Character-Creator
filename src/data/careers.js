export default {
    Entertainer: {
        name: 'Entertainer',
        description: 'See CRB, page 40, for more details.',
        book: 'CRB',
        page: '40',
        setting: 'All',
        skills: ['Charm', 'Coordination', 'Deception', 'Discipline', 'Leadership', 'Melee', 'MeleeLight', 'Skullduggery', 'Stealth']
    },
    Explorer: {
        name: 'Explorer',
        description: 'See CRB, page 41, for more details.',
        book: 'CRB',
        page: '41',
        setting: 'All',
        skills: ['Athletics', 'Brawl', 'Coordination', 'Deception', 'Perception', 'Ranged', 'RangedHeavy', 'Stealth', 'Survival']
    },
    Healer: {
        name: 'Healer',
        description: 'See CRB, page 41, for more details.',
        book: 'CRB',
        page: '41',
        setting: 'All',
        skills: ['Cool', 'Discipline', 'Knowledge', 'Medicine', 'Melee', 'MeleeLight', 'Resilience', 'Survival', 'Vigilance']
    },
    Leader: {
        name: 'Leader',
        description: 'See CRB, page 41, for more details.',
        book: 'CRB',
        page: '41',
        setting: 'All',
        skills: ['Charm', 'Coercion', 'Cool', 'Discipline', 'Leadership', 'Melee', 'MeleeLight', 'Negotiation', 'Perception']
    },
    Scoundrel: {
        name: 'Scoundrel',
        description: 'See CRB, page 41, for more details.',
        book: 'CRB',
        page: '41',
        setting: 'All',
        skills: ['Charm', 'Cool', 'Coordination', 'Deception', 'Ranged', 'RangedLight', 'Skullduggery', 'Stealth', 'Streetwise']
    },
    Socialite: {
        name: 'Socialite',
        description: 'See CRB, page 41, for more details.',
        book: 'CRB',
        page: '41',
        setting: 'All',
        skills: ['Charm', 'Cool', 'Deception', 'Knowledge', 'Negotiation', 'Perception', 'Streetwise', 'Vigilance']
    },
    Soldier: {
        name: 'Soldier',
        description: 'See CRB, page 41, for more details.',
        book: 'CRB',
        page: '41',
        setting: 'All',
        skills: ['Athletics', 'Brawl', 'Gunnery', 'Coercion', 'Melee', 'MeleeHeavy', 'Perception', 'Ranged', 'RangedHeavy', 'Survival', 'Vigilance']
    },
    Tradesperson: {
        name: 'Tradesperson',
        description: 'See CRB, page 42, for more details.',
        book: 'CRB',
        page: '41',
        setting: 'All',
        skills: ['Athletics', 'Brawl', 'Discipline', 'Mechanics', 'Negotiation', 'Perception', 'Resilience', 'Streetwise']
    },
    Hacker: {
        name: 'Hacker',
        setting: 'Science Fiction, Space Opera, Any Setting With Hacking Alternate Rules',
        book: 'CRB',
        page: '42',
        description: 'See CRB, page 42, for more details.',
        skills: ['Computers', 'Discipline', 'Knowledge', 'Mechanics', 'Perception', 'Piloting', 'Streetwise', 'Vigilance']
    },
    FighterPilot: {
        name: 'Fighter Pilot',
        setting: 'Weird War, Modern, Science Fiction, Space Opera, Any Setting Using Vehicle Alternate Rules',
        book: 'CRB',
        page: '42',
        description: 'See CRB, page 42, for more details.',
        skills: ['Cool', 'Driving', 'Gunnery', 'Mechanics', 'Perception', 'Piloting', 'RangedLight', 'Vigilance']
    },
    Knight: {
        name: 'Knight',
        setting: 'Fantasy',
        book: 'CRB',
        page: '42',
        description: 'See CRB, page 42, for more details.',
        skills: ['Athletics', 'Discipline', 'Leadership', 'MeleeHeavy', 'MeleeLight', 'Resilience', 'Riding', 'Vigilance', 'Melee', 'Gunnery', 'Driving', 'Piloting', 'Operating']
    },
    MadScientist: {
        name: 'Mad Scientist',
        setting: 'Steampunk, Weird War',
        book: 'CRB',
        page: '43',
        description: 'See CRB, page 43, for more details.',
        skills: ['Alchemy', 'Coercion', 'Knowledge', 'Mechanics', 'Medicine', 'Operating', 'Skullduggery', 'RangedHeavy', 'Arcana']
    },
    Priest: {
        name: 'Priest',
        setting: 'Any Setting Using the Magic Alternate Rules',
        book: 'CRB',
        page: '43',
        description: 'See CRB, page 43, for more details.',
        skills: ['Charm', 'Coercion', 'Cool', 'Discipline', 'Divine', 'Medicine', 'Melee', 'MeleeLight', 'Negotiation']
    },
    Druid: {
        name: 'Druid',
        setting: 'Any Setting Using the Magic Alternate Rules',
        book: 'CRB',
        page: '43',
        description: 'See CRB, page 43, for more details.',
        skills: ['Athletics', 'Brawl', 'Coordination', 'Melee', 'MeleeHeavy', 'Primal', 'Resilience', 'Survival', 'Vigilance']
    },
    StarshipCaptain: {
        name: 'Starship Captain',
        setting: 'Science Fiction, Space Opera, Any Setting Using Vehicle Alternate Rules',
        book: 'CRB',
        page: '43',
        description: 'See CRB, page 43, for more details.',
        skills: ['Computers', 'Discipline', 'Gunnery', 'Knowledge', 'Leadership', 'Mechanics', 'Operating', 'Perception', 'Astrocartography']
    },
    Wizard: {
        name: 'Wizard',
        setting: 'Any Setting Using the Magic Alternate Rules',
        book: 'CRB',
        page: '43',
        description: 'See CRB, page 43, for more details.',
        skills: ['Arcana', 'Coercion', 'Discipline', 'Knowledge', 'Leadership', 'Skullduggery', 'Stealth', 'Vigilance', 'Alchemy']
    },

    //Realms of Terrinoth

    Disciple: {
        name: 'Disciple',
        setting: 'Fantasy',
        book: 'ROT',
        page: '70',
        description: 'See ROT, page 70, for more details.',
        skills: ['Athletics', 'Charm', 'Discipline', 'Divine', 'KnowledgeLore', 'Leadership', 'MeleeLight', 'Resilience']
    },
    Envoy: {
        name: 'Envoy',
        setting: 'Fantasy',
        book: 'ROT',
        page: '70',
        description: 'See ROT, page 70, for more details.',
        skills: ['Charm', 'Cool', 'Deception', 'KnowledgeGeography', 'Leadership', 'MeleeLight', 'Negotiation', 'Vigilance']
    },
    Mage: {
        name: 'Mage',
        setting: 'Fantasy',
        book: 'ROT',
        page: '71',
        description: 'See ROT, page 71, for more details.',
        skills: ['Alchemy', 'Cool', 'Arcana', 'Discipline', 'KnowledgeAdventuring', 'KnowledgeForbidden', 'KnowledgeLore', 'Perception']
    },
    Primalist: {
        name: 'Primalist',
        setting: 'Fantasy',
        book: 'ROT',
        page: '71',
        description: 'See ROT, page 71, for more details.',
        skills: ['Alchemy', 'Brawl', 'Discipline', 'KnowledgeLore', 'Medicine', 'MeleeHeavy', 'Primal', 'Survival']
    },
    Scholar: {
        name: 'Scholar',
        setting: 'Fantasy',
        book: 'ROT',
        page: '72',
        description: 'See ROT, page 72, for more details.',
        skills: ['Alchemy', 'KnowledgeForbidden', 'KnowledgeGeography', 'KnowledgeLore', 'Mechanics', 'Medicine', 'Perception', 'Runes']
    },
    Scout: {
        name: 'Scout',
        setting: 'Fantasy',
        book: 'ROT',
        page: '73',
        description: 'See ROT, page 73, for more details.',
        skills: ['KnowledgeAdventuring', 'KnowledgeGeography', 'Perception', 'Ranged', 'Riding', 'Stealth', 'Survival', 'Vigilance']
    },
    Warrior: {
        name: 'Warrior',
        setting: 'Fantasy',
        book: 'ROT',
        page: '73',
        description: 'See ROT, page 73, for more details.',
        skills: ['Brawl', 'Coercion', 'Leadership', 'MeleeHeavy', 'MeleeLight', 'Resilience', 'Riding', 'Vigilance']
    },

}

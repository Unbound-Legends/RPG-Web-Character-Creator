...
import {diceNames} from '../../data/lists'
...
class CustomTalentsComponent extends React.Component {
	...
	render() {
		...
		return <div>
			...
			<Fragment type='inputSelect' title='tier'
					  array={[1, 2, 3, 4, 5]}
					  value={tier}
					  handleChange={(event) => this.setState({tier: +event.target.value})}/>

			<Fragment type='inputSelect' title='activation'
					  array={[true, false]} nameObj={{true: {name: 'Active'}, false: {name: 'Passive'}}}
					  value={activation}
					  handleChange={(event) => this.setState({activation: event.target.value})}/>


			{activation && <Fragment type='text' title='turn' value={turn}
									 handleChange={(event) => this.setState({turn: event.target.value})}/>}

			<Fragment type='inputSelect' title='ranked'
					  array={[true, false]} nameObj={{true: {name: 'Yes'}, false: {name: 'No'}}}
					  value={ranked}
			...
			<Fragment type='inputSelect' title='prerequisite' value={prerequisite} array={Object.keys(talents)}
					  nameObj={talents}
					  blankText={'None'}
					  handleChange={(event) => this.setState({prerequisite: event.target.value})}/>

			<Fragment type='inputSelect' title='antirequisite' value={antirequisite} array={Object.keys(talents)}
					  nameObj={talents}
					  blankText={'None'}
					  handleChange={(event) => this.setState({antirequisite: event.target.value})}/>

			<Fragment type='inputSelect' title='modifier' array={[true, false]}
					  nameObj={{true: {name: 'Yes'}, false: {name: 'No'}}}
					  value={modifier} blankOption={false}
					  handleChange={(event) => this.setState({modifier: event.target.value, modifierValue: ''})}/>
			...
			(modifier === 'careerSkills' ?
				<Fragment type='inputSelect' title='modifierValue' value=''
						  array={Object.keys(skills).filter(skill => !modifierValue.includes(skill))}
						  nameObj={skills}
			...
			<Fragment type='list' title='modifierValue' array={modifierValue} nameObj={{...skills, diceNames}}

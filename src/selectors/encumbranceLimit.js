...
const calcEncumbranceLimit = createSelector(
	selectors.characteristics, equipmentGear, gear, talents, selectors.talentCount,
	(characteristics, equipmentGear, gear, talents, talentCount) => {
		const Brawn = characteristics.Brawn;

		//get gear modifier
		const gearModifier = Object.keys(equipmentGear).map(item => {
		...
		//add other talents
		const talentsModifier = Object.entries(talentCount).map(([talent, count]) => {
			return get(talents, `${talent}.modifier.maxEncumbrance`, 0) * count;
		}).reduce((acc, val) => acc + val, 0);
